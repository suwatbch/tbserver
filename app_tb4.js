...
async function acceptJob(page, row) {
    try {
        ...
        try {
            ...
            // รอให้ popup แสดงผลสมบูรณ์
            await new Promise(resolve => setTimeout(resolve, 2000));
            
            // ตรวจสอบสถานะของ input
            const checkInput = await page.evaluate(() => {
                const input = document.querySelector('input[name="cf-turnstile-response"]');
                // ตรวจสอบว่ามี input และ value ไม่เป็นค่าว่าง
            ...
            const popupClosed = await Promise.race([
                page.waitForSelector('.el-dialog__wrapper[flag="true"]', { 
                    hidden: true,
                    timeout: 30000 
                }).then(() => true),
            ...
            // ถ้าครบ 30 วินาทีและ popup ยังไม่หายไป ให้กดปุ่มปิด
            if (!popupClosed) {
                console.log('ครบ 30 วินาที กำลังกดปุ่มปิด Popup...');
                await page.evaluate(() => {
                    const closeButton = document.querySelector('.el-dialog__wrapper[flag="true"] .el-dialog__headerbtn');
                    if (closeButton) {
                        closeButton.click();
                    }
                });
                
                // รอให้ popup หายไปหลังจากกดปิด
                await page.waitForSelector('.el-dialog__wrapper[flag="true"]', { 
                    hidden: true,
                    timeout: 3000 
                });
